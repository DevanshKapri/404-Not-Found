...
export const GraphProvider = ({ children }) => {
  ...
  const PredictionData = async ({ days, symbol }) => {
    try {
      fetch('https://crypto404notfound.herokuapp.com/predict/', {
        method: 'POST',
        headers: {
          Accept: 'application/json, text/plain, */*',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ days: days, symbol: symbol }),
      })
        .then((res) => res.json())
        .then((res) => {
          console.log('res', res);
  ...
  function fetchHistory({ Dur }) {
    ...
    setFetching(true);
    fetch(
      `https://coinranking1.p.rapidapi.com/coin/${Coin.uuid}/history?referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=${Dur}`,
      {
        method: 'GET',
        headers: {
          'x-rapidapi-host': 'coinranking1.p.rapidapi.com',
          'x-rapidapi-key':
            '527a722143msh647fbcb572b6a8fp1fd8e9jsn86102cfb3192',
        },
      }
    )
      ...
      .then((res) => {
        ...
        for (var i = 0; i < res.data.history.length; i++) {
          ...
          Tl.push(
            moment.unix(res.data.history[i].timestamp).format('DD MMM YY')
          );
        }
