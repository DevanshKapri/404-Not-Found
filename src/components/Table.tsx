...
const getData = async () => {
  try {
    ...
    console.log(data);
    data.map((d: any) => {
      const temp: Data = createData(d.rank, d.id, d.name, `$${d.price}`, millify(d.market_cap), `${d['7d'].price_change_pct}%`, `${d['7d'].volume_change_pct}%`, 'pr');
      rows.push(temp);
    })
  } catch (err) {
    console.log(err);
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right">{row.priceChangePct}</TableCell>
                      <TableCell align="right">{row.volumeChangePct}</TableCell>
